...
/** Fetch og:image thumbnail for one album (cached) */
async function fetchAlbumThumb(albumUrl, storeUrl) {
  const key = `thumb:${albumUrl}`;
  const hit = cache.get(key);
  if (hit !== undefined) return hit;
  try {
    const html = await Promise.race([
      fetchHtml(albumUrl, storeUrl),
      new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), 6000)),
    ]);
    // og:image — handle both https:// and // protocol-relative
    let thumb = '';
    const ogM = html.match(/property=["']og:image["'][^>]*content=["']([^"']+)["']/i)
             || html.match(/content=["']([^"']+)["'][^>]*property=["']og:image["']/i);
    if (ogM) thumb = ogM[1];
    // fallback: first photo.yupoo.com URL in the page
    if (!thumb) {
      const phM = html.match(/(?:https?:)?\/\/photo\.yupoo\.com\/[^\s"'<>)]+/i);
      if (phM) thumb = phM[0];
    }
    if (thumb.startsWith('//')) thumb = 'https:' + thumb;
    cache.set(key, thumb, 600);
    return thumb;
  } catch (_) {
    cache.set(key, '', 60);
    return '';
...
/** Page 1 of store — fast load (profile + first batch of albums + thumbnails) */
app.get('/api/store', async (req, res) => {
  ...
  try {
    ...
    // Prefetch thumbnails for ALL albums in parallel (batches of 8)
    const batchSize = 8;
    for (let i = 0; i < albums.length; i += batchSize) {
      const batch = albums.slice(i, i + batchSize);
      await Promise.all(batch.map(async (album) => {
        if (!album.thumbnail) {
          album.thumbnail = await fetchAlbumThumb(album.url, info.storeUrl);
        }
      }));
    }
...
app.get('/api/albums', async (req, res) => {
  ...
  try {
    ...
    const albums = parseAlbums($, info.username, html);
    const pagination = parsePagination($);
    // Prefetch thumbnails for this batch too
    const batchSize = 8;
    for (let i = 0; i < albums.length; i += batchSize) {
      const batch = albums.slice(i, i + batchSize);
      await Promise.all(batch.map(async (album) => {

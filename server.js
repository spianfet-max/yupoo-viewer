...
/** Parse album cards from a store page */
function parseAlbums($, username) {
  const albums = [];
  const seen   = new Set();
  ...
  $('a[href*="/albums/"]').each((_, el) => {
    ...
    if (!clean.includes('/albums/') || seen.has(clean)) return;
    seen.add(clean);

    const albumUrl = clean.startsWith('http')
      ? clean
      : `https://${username}.yupoo.com${clean}`;
    ...
    // Title: look upward for album info containers
    const $card = $el.closest('[class*="album"]');
    ...
    albums.push({ url: albumUrl, thumbnail: thumbnailUrl(thumb), title, count });
  });
...
app.get('/api/store', async (req, res) => {
  ...
  try {
    ...
    const albums      = parseAlbums($, info.username);
...
app.get('/api/album', async (req, res) => {
  ...
  try {
    const fetchUrl       = `${info.albumUrl}?page=${page}`;
    const { data: html } = await http.get(fetchUrl, {
      headers: { Referer: info.storeUrl },
    });
